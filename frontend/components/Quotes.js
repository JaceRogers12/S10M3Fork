...
import {toggleVisibility, deleteQuote, editQuoteAuthenticity, setHighlightedQuote} from "../state/quotesSlice.js";

export default function Quotes() {
  const quotes = useSelector(store => store.state.quotes);
  const displayAllQuotes = useSelector(store => store.state.displayAllQuotes);
  const highlightedQuote = useSelector(store => store.state.highlightedQuote);
